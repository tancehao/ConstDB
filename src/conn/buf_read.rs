...
impl ReadBuf {
    ...
    #[inline]
    pub fn read_buf_cap(&self) -> usize {
    ...
    fn after_parsed(&mut self) {
        if self.input_parsed == self.input_pos {
            self.input_parsed = 0;
            self.input_pos = 0;
        } else if self.input_pos == self.input_buf.len() + 1
            && (self.input_pos - self.input_parsed) <= self.input_pos / 2
        {
        ...
        match self.input_buf.get_mut(cursor) {
            ...
            Some(t) => match *t {
                RESP_TYPE_STRING => {
                    ...
                    Ok((
                        Message::String(self.input_buf[cursor + 1..s - 1].into()),
                        s - cursor + 1,
                    ))
                }
                RESP_TYPE_INTEGER => {
                    ...
                    match bytes2i64(self.input_buf[cursor + 1..s - 1].as_ref()) {
                        ...
                        None => Err(CstError::InvalidRequestMsg(
                            "the ':' should be followed by an integer".to_string(),
                        )),
                    }
                }
                RESP_TYPE_ERR => {
                    let s = self.read_until_crcf(cursor + 1)?;
                    Ok((
                        Message::Error(self.input_buf[cursor + 1..s - 1].into()),
                        s - cursor + 1,
                    ))
                }
                RESP_TYPE_BULK => {
                ...
                other => {
                    error!(
                        "unknown resp type {}, buf: {:?}",
                        other,
                        self.input_buf[cursor..].as_ref()
                    );
                    Err(CstError::InvalidRequestMsg(format!(
                        "unknown resp type {}",
                        other
                    )))
                }
            ...
            Some(t) => {
                if *t != RESP_TYPE_BULK {
                    Err(CstError::InvalidRequestMsg(
                        "should be of bulk string type".to_string(),
                    ))
                } else {
                    ...
                    match bytes2i64(self.input_buf[cursor + 1..header_end - 1].as_ref()) {
                        ...
                        Some(char_cnt) => {
                            ...
                            if str_end - header_end != char_cnt as usize + 2 {
                                return Err(CstError::InvalidRequestMsg(
                                    "bulk string has wrong value of length".to_string(),
                                ));
                            }
...
impl TryReader for Vec<u8> {
    fn try_read(&self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        buf[0..l]
            .as_mut()
            .copy_from_slice(self.as_slice()[0..l].as_ref());
        Ok(l)
    }
}
